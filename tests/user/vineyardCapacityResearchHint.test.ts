...
describe('getNextVineyardCapacityResearch', () => {
  it('treats 0.25 hectares as the base cap and points to the next research tier', () => {
    const nextProject = getNextVineyardCapacityResearch('vineyard_size', 0.25);

    expect(nextProject?.id).toBe('eff_smallholding_operations');
  });
  ...
  it('returns the next total-area chain research from the base cap', () => {
    const nextProject = getNextVineyardCapacityResearch('total_vineyard_hectares', 0.25);

    expect(nextProject?.id).toBe('eff_total_land_budgeting');
  });
