...
describe('Storage Vessel capacity summary', () => {
  it('does not count vessels in maintenance as available capacity', () => {
  ...
  it('counts dirty vessels as available capacity while cleanliness is warning-only', () => {
    const summary = calculateStorageCapacitySummary([
      vessel('clean', 500, 'available'),
      { ...vessel('dirty', 250, 'available'), cleanliness: 'dirty' },
    ]);
    expect(summary.availableLitres).toBe(750);
    expect(summary.availableVesselCount).toBe(2);
  });
});
