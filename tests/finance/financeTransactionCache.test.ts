...
const mocks = vi.hoisted(() => {
  ...
  const historicalTransaction: Transaction = {
    ...
    date: { week: 1, season: 'Spring', year: 2026 },
    amount: 500,
  ...
  const newTransaction: Transaction = {
    ...
    date: { week: 2, season: 'Spring', year: 2026 },
    amount: persistedTransaction.amount,
    description: persistedTransaction.description,
    category: persistedTransaction.category,
    recurring: persistedTransaction.recurring,
    money: persistedTransaction.money,
  };
...
vi.mock('@/lib/services/user/companyService', () => ({
  companyService: { getCompany: mocks.getCompany },
}));
...
vi.mock('@/lib/features/loanLender', () => ({
  loanLenderFeature: { metrics: { calculateTotalOutstandingLoans: vi.fn(async () => 0) } },
}));
vi.mock('@/lib/services/wine/winescore/wineScoreCalculation', () => ({
  calculateLandValuePriceMultiplier: vi.fn(() => 1),
}));
