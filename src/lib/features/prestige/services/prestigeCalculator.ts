...
export function softCapSigned(amount: number, cap: number): number {
  ...
  return (
    Math.sign(amount) * safeCap * (1 - Math.exp(-Math.abs(amount) / safeCap))
  );
}

export function calculateFeatureSeverityPrestigeMultiplier(
  baseAmount: number,
  severity: number = 1,
): number {
...
export function calculateSalePrestigeWithAssets(
  ...
  const assetsFactor =
    Math.sqrt(Math.max(1, companyAssets) / 100000) * assetsWeight;
...
export function calculateFeatureSalePrestigeWithReputation(
  ...
  const prestigeFactor =
    (Math.log(
      1 / (1 - NormalizeScrewed1000To01WithTail(companyPrestige) + 0.001),
    ) /
      5) *
    prestigeWeight;

  // Formula: baseAmount × (volume + value) × prestige
  const severityFactor = calculateFeatureSeverityPrestigeMultiplier(
    baseAmount,
    featureSeverity,
  );
  const amount =
    baseAmount * (volumeFactor + valueFactor) * prestigeFactor * severityFactor;

  // Cap using provided maxImpact or default
  const cap =
    maxImpact !== undefined ? maxImpact : baseAmount < 0 ? -10.0 : 10.0;
  return baseAmount < 0
...
export function calculateVineyardManifestationPrestige(
  ...
  const vineyardFactor =
    (Math.log(
      1 / (1 - NormalizeScrewed1000To01WithTail(vineyardPrestige) + 0.001),
    ) /
      5) *
    vineyardPrestigeWeight;
  ...
  const cap =
    maxImpact !== undefined ? maxImpact : baseAmount < 0 ? -10.0 : 10.0;
  return baseAmount < 0 ? Math.max(cap, amount) : Math.min(cap, amount);
}
...
export function calculateVineyardSalePrestige(
  ...
  const vineyardPrestigeFactor =
    0.5 + 1.5 * Normalize1000To01WithTail(vineyardPrestige);
  const rawAmount =
    Math.max(0, baseAmount) *
    (volumeFactor + valueFactor) *
