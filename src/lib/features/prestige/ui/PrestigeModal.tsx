...
import {
  getEventDisplayData,
  consolidateWineFeatureEvents,
  ConsolidatedWineFeatureEvent,
} from '@/lib/features/prestige/services/prestigeService';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
const PrestigeModal: React.FC<PrestigeModalProps> = ({
  ...
  const eventConfig = {
    ...
    vineyard_sale: {
      icon: Grape,
      label: 'Vineyard Sales',
      color: 'bg-green-100 text-green-800',
    },
    vineyard_achievement: {
      icon: Star,
      label: 'Vineyard Achievements',
      color: 'bg-yellow-100 text-yellow-800',
    },
    vineyard_age: {
      icon: Star,
  ...
  const formatProjectionAmount = (
    value: number,
    sourceCurrentValue: number,
  ) => {
    const safeValue = Number.isFinite(value) ? value : 0;
    const safeCurrent = Number.isFinite(sourceCurrentValue)
      ? sourceCurrentValue
      : 0;
  ...
  const getAchievementCategoryKey = (event: PrestigeEvent): string => {
    ...
    if (
      typeof metadata.achievementCategory === 'string' &&
      metadata.achievementCategory.trim()
    ) {
  ...
  const getAchievementCategoryLabel = (categoryKey: string): string => {
    ...
    return (
      labels[categoryKey] ??
      `${categoryKey.replace(/_/g, ' ').replace(/\b\w/g, (l) => l.toUpperCase())} Achievements`
    );
  };

  // Memoized CalculationTooltip component
  const CalculationTooltip = useCallback(
    ({
      children,
      calculationData,
    }: {
          ...
          case 'company_value':
            return (
              <div className="space-y-2">
                <TooltipRow
                  label="Company Value:"
                  value={formatNumber(calculationData.companyValue, {
                    currency: true,
                    smartDecimals: true,
                  })}
                  valueRating={getRatingForRange(
                    calculationData.companyValue / 1000000,
                    0,
                    1000,
                    'exponential',
                  )}
                ...
                <TooltipRow
                  ...
                  value={formatNumber(calculationData.maxLandValue, {
                    currency: true,
                    smartDecimals: true,
                  })}
                />
                ...
                <TooltipRow
                  ...
                  value={formatNumber(calculationData.finalPrestige, {
                    smartDecimals: true,
                  })}
                  valueRating={getRatingForRange(
                    calculationData.finalPrestige,
                    0,
                    10,
                    'higher_better',
                  )}
            ...
            return (
              <div className="space-y-2">
                ...
                <TooltipRow
                  ...
                  valueRating={getRatingForRange(
                    calculationData.landValuePerHa,
                    0,
                    1000000,
                    'exponential',
                  )}
                ...
                <TooltipRow
                  ...
                  value={formatNumber(calculationData.landNormalized01, {
                    smartDecimals: true,
                  })}
                  valueRating={getRatingForRange(
                    calculationData.landNormalized01,
                    0,
                    1,
                    'higher_better',
                  )}
                ...
                <TooltipRow
                  ...
                  value={formatNumber(calculationData.landWithSuitability01, {
                    smartDecimals: true,
                  })}
                  valueRating={getRatingForRange(
                    calculationData.landWithSuitability01,
                    0,
                    1,
                    'higher_better',
                  )}
                ...
                <TooltipRow
                  label="Size Factor:"
                  value={formatNumber(calculationData.sizeFactor, {
                    smartDecimals: true,
                  })}
                />
                <TooltipRow
                  label="Asymmetric Scaling:"
                  value={formatNumber(calculationData.asymScaling, {
                    smartDecimals: true,
                  })}
                />
                <TooltipRow
                ...
                <div className="mt-2 border-t border-gray-600 pt-2 space-y-2 text-xs text-gray-400">
                  ...
                  <div>
                    <div>
                      Density Modifier: [Vineyard Density / Default Density]
                    </div>
                  </div>
                  {(() => {
                    const normalized = formatNumber(
                      calculationData.landNormalized01,
                      { smartDecimals: true },
                    );
                    const suitability = formatNumber(
                      calculationData.suitability,
                      { smartDecimals: true },
                    );
                    const withSuitability = formatNumber(
                      calculationData.landWithSuitability01,
                      { smartDecimals: true },
                    );
                    const perHaAsym = formatNumber(
                      calculationData.asymScaling,
                      { smartDecimals: true },
                    );
                    const size = formatNumber(calculationData.sizeFactor, {
                      smartDecimals: true,
                    });
                    const densityMod = formatNumber(
                      calculationData.densityModifier,
                      { smartDecimals: true },
                    );
                    const final = formatNumber(calculationData.finalPrestige, {
                      smartDecimals: true,
                    });
                    return (
                      <div>
          ...
          case 'vineyard_age':
            return (
              <div className="space-y-2">
                ...
                <TooltipRow
                  label="Age Base:"
                  value={formatNumber(calculationData.ageBase, {
                    smartDecimals: true,
                  })}
                  valueRating={getRatingForRange(
                    calculationData.ageBase,
                    0,
                    1,
                    'higher_better',
                  )}
                ...
                <TooltipRow
                  ...
                  value={formatNumber(calculationData.grapeSuitability, {
                    smartDecimals: true,
                  })}
                  valueRating={getRatingForRange(
                    calculationData.grapeSuitability,
                    0,
                    1,
                    'higher_better',
                  )}
                ...
                <div className="mt-2 border-t border-gray-600 pt-2 space-y-2 text-xs text-gray-400">
                  ...
                  <div>
                    <div>
                      Age Modifier: Converts vine age to 0-1 scale with
                      diminishing returns
                    </div>
                  </div>
                  {(() => {
                    const ageBase = formatNumber(calculationData.ageBase, {
                      smartDecimals: true,
                    });
                    const suitability = formatNumber(
                      calculationData.grapeSuitability,
          ...
          case 'wine_feature':
            return (
              <div className="space-y-2">
                ...
                />
                <TooltipRow label="Wine:" value={calculationData.wineName} />
                <TooltipRow
                  label="Event Type:"
                  value={calculationData.eventType}
                />
                <TooltipRow label="Level:" value={calculationData.level} />
                <TooltipRow
                  label="Base Amount:"
                  value={formatNumber(calculationData.baseAmount, {
                    smartDecimals: true,
                  })}
                  valueRating={getRatingForRange(
                ...
                <div className="mt-2 border-t border-gray-600 pt-2 space-y-2 text-xs text-gray-400">
                  ...
                  <div>
                    <div>
                      Dynamic Scaling: May include company/vineyard prestige,
                      batch size, quality, and sale value factors
                    </div>
                  </div>
                  {(() => {
                    const baseAmount = formatNumber(
                      calculationData.baseAmount,
                      { smartDecimals: true },
                    );
                    const final = formatNumber(calculationData.finalPrestige, {
  ...
  const estimateWeeksFromDecay = (
    ...
    if (Math.abs(originalAmount) < 0.001 || Math.abs(currentAmount) < 0.001)
      return undefined;
  ...
  const selectProjectionHorizons = (
    decayRate: number,
    ...
    const maxRelevantWeeks =
      onePercentWeeks < 104
        ? Math.max(26, Math.ceil(onePercentWeeks * 1.25))
        : Infinity;

    const preferred = projectionCandidates.filter(
      (candidate) =>
        preferredWeeks.includes(candidate.weeks) &&
        candidate.weeks <= maxRelevantWeeks,
    );
    ...
    const targets = [0.25, 0.5, 1, 2, 4].map((multiplier) =>
      Math.max(1, Math.round(halfLifeWeeks * multiplier)),
    );
    ...
    for (const target of targets) {
      ...
      for (const candidate of projectionCandidates) {
        if (used.has(candidate.weeks) || candidate.weeks > maxRelevantWeeks)
          continue;
  ...
  const EventDisplay = ({ event }: { event: PrestigeEvent }) => {
    ...
    const isAchievement =
      event.type === 'achievement' || event.type === 'vineyard_achievement';
    const currentValue = event.currentAmount ?? event.amount;
    const hasDecay =
      Number.isFinite(event.decayRate) &&
      event.decayRate > 0 &&
      event.decayRate < 1;
    const decayProjection = hasDecay
      ? buildDecayProjection(currentValue, event.decayRate)
      : [];
      ...
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 flex-wrap">
            {isAchievement ? (
              <p className={`text-sm ${tooltipStyles.subtitle}`}>
                {displayData.title}
              </p>
            ) : displayData.calculationData ? (
              <CalculationTooltip calculationData={displayData.calculationData}>
                <p className={`text-sm ${tooltipStyles.subtitle} cursor-help`}>
                  {displayData.title}
                </p>
              </CalculationTooltip>
            ) : (
            ...
            {!isAchievement && displayData.displayInfo && (
                  ...
                  <div className={tooltipStyles.text}>
                    <TooltipSection>
                      <TooltipHeader
                        ...
                        description={
                          displayData.displayInfo || 'Additional information'
                        }
                      />
                    </TooltipSection>
              ...
              >
                <span
                  className={`${tooltipStyles.text} text-muted-foreground cursor-help`}
                >
                  (details)
                </span>
              </UnifiedTooltip>
        ...
        <div className="text-right">
          <UnifiedTooltip
            content={(() => {
              ...
              const weeks = estimateWeeksFromDecay(
                original,
                current,
                event.decayRate,
              );
                  ...
                  <TooltipSection title="Prestige Details">
                    ...
                    <TooltipRow
                      ...
                      value={
                        event.decayRate === 0
                          ? 'No decay'
                          : `${formatNumber((1 - event.decayRate) * 100, { smartDecimals: true })}%`
                      }
                      ...
                      valueRating={
                        event.decayRate === 0
                          ...
                          : Math.max(
                              0,
                              Math.min(1, (event.decayRate - 0.9) / 0.1),
                            )
                      }
                    />
                    {event.decayRate &&
          ...
          >
            <p
              className={`text-sm ${tooltipStyles.subtitle} cursor-help ${getRangeColor(event.currentAmount ?? event.amount, 0, 10, 'higher_better').text}`}
            >
  ...
  const ConsolidatedWineFeatureDisplay = ({
    consolidatedEvent,
  }: {
    consolidatedEvent: ConsolidatedWineFeatureEvent;
  }) => {
    const {
      vineyardName,
      grape,
              ...
              <div className="flex items-center gap-2">
                      ...
                      <TooltipSection title={`${feature.featureName} Events`}>
                        <div className="space-y-2">
                          {feature.recentEvents.map((event, eventIdx) => {
                            ...
                            const customerName =
                              metadata.customerName || 'Unknown Customer';
                ...
                >
                  <Badge variant="outline" className="text-[10px] cursor-help">
                    {feature.eventCount}{' '}
                    {feature.eventCount === 1 ? 'event' : 'events'}
                  </Badge>
                </UnifiedTooltip>
              </div>
              <div className="text-right">
                <span className="font-medium">
                  {formatAmount(feature.totalAmount)}
                </span>
                <span className="text-gray-500 ml-1">
                  ({formatDecayRate(feature.decayRate)})
                </span>
              </div>
            </div>
          ))}
  ...
  const getFilteredVineyards = () =>
    selectedVineyard === 'all'
      ? vineyards
      : vineyards.filter((vineyard) => vineyard.id === selectedVineyard);

  const getFilteredWines = () => {
    if (selectedWine === 'all') {
      return consolidatedCompanyWineFeatures;
    }
    return consolidatedCompanyWineFeatures.filter(
      (wine) =>
  ...
  const allWineFeatureEvents = eventBreakdown.filter(
    (event) => event.type === 'wine_feature',
  );
  ...
  const consolidatedCompanyWineFeatures = consolidateWineFeatureEvents(
    companyWineFeatureEvents,
  );
  const consolidatedVineyardWineFeatures = consolidateWineFeatureEvents(
    vineyardWineFeatureEvents,
  );

  // Group other company events by type (including research achievements)
  const otherCompanyEvents = eventBreakdown.filter((event) =>
    [
      'company_finance',
      'company_story',
      'sale',
  ...
  const groupedCompanyEvents = otherCompanyEvents.reduce(
    (acc, event) => {
  ...
  React.useEffect(() => {
    ...
    vineyards.forEach((vineyard) => {
      const vineyardWineFeatures = consolidatedVineyardWineFeatures.filter(
        (wine) =>
          wine.vineyardId === vineyard.id ||
          wine.vineyardName === vineyard.name,
      );
    ...
    groupedCompanyEvents,
    vineyards,
    consolidatedCompanyWineFeatures,
    consolidatedVineyardWineFeatures,
  ]);
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto scrollbar-styled">
        <DialogHeader>
          ...
          <DialogDescription>
            View detailed breakdown of your company's prestige from various
            sources including sales, vineyards, and company value.
          </DialogDescription>
        </DialogHeader>
            ...
            <CardContent>
              <div className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Your company's total prestige is calculated from various
                  sources. Some prestige sources decay over time, while others
                  remain constant.
                </p>
                      ...
                      <div className="space-y-3">
                        ...
                        >
                          ...
                          <Badge className="bg-purple-100 text-purple-800">
                            {consolidatedCompanyWineFeatures.length}{' '}
                      ...
                      <CardContent>
                        <div className="space-y-2">
                          {consolidatedCompanyWineFeatures
                            ...
                            .map((wine) => (
                              <div
                                key={`${wine.vineyardId}_${wine.grape}_${wine.vintage}`}
                              ...
                              >
                                <div className="flex items-center gap-2">
                                  ...
                                  >
                                    ...
                                    >
                                      {wine.features.length}{' '}
                                      {wine.features.length === 1
                                        ? 'feature'
                                        : 'features'}
                                    </Badge>
                                  </UnifiedTooltip>
                    ...
                    <CardHeader>
                      <div className="space-y-3">
                        ...
                        >
                          <Star className="h-4 w-4 text-purple-600" />
                          {(() => {
                            const wine = consolidatedCompanyWineFeatures.find(
                              (w) =>
                                `${w.vineyardName}_${w.grape}_${w.vintage}` ===
                                selectedWine,
                            );
                            return wine
                    ...
                    {!isSectionCollapsed(`wine_${selectedWine}`) && (
                      <CardContent className="space-y-3">
                        {getFilteredWines().map((consolidatedEvent, index) => (
                          <div
                            key={`${consolidatedEvent.vineyardId}_${consolidatedEvent.grape}_${consolidatedEvent.vintage}`}
                          >
                            <ConsolidatedWineFeatureDisplay
                              consolidatedEvent={consolidatedEvent}
                            />
                            {index < getFilteredWines().length - 1 && (
                      ...
                      <CardHeader>
                        ...
                        >
                          ...
                          <Badge className={getEventConfig(type).color}>
                            {events.length}{' '}
                            {events.length === 1 ? 'source' : 'sources'}
                          </Badge>
                          {events.length > 1 && (
                            <>
                              {isCollapsed ? (
                        ...
                        <CardContent className="space-y-3">
                          {type === 'achievement'
                            ? // Group achievements by category
                              (() => {
                                const achievementsByCategory = events.reduce(
                                  (acc, event) => {
                                    const categoryKey =
                                      getAchievementCategoryKey(event);
                                    if (!acc[categoryKey])
                                      acc[categoryKey] = [];
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      <Badge className="bg-green-100 text-green-800">
                        {vineyards.length}{' '}
                        {vineyards.length === 1 ? 'vineyard' : 'vineyards'}
                      </Badge>
                      {vineyards.length > 1 && (
                        <>
                ...
                <CardHeader>
                  <div className="space-y-3">
                    <CardTitle
                      className="flex items-center gap-2 text-base cursor-pointer hover:bg-gray-50 p-2 -m-2 rounded transition-colors"
                      onClick={() =>
                        toggleSection(`vineyard_${selectedVineyard}`)
                      }
                    >
                      <Grape className="h-4 w-4 text-green-600" />
                      {(() => {
                        const vineyard = vineyards.find(
                          (v) => v.id === selectedVineyard,
                        );
                ...
                {!isSectionCollapsed(`vineyard_${selectedVineyard}`) && (
                  <CardContent className="space-y-3">
                    {getFilteredVineyards().map((vineyard) => {
                      ...
                      const vineyardWineFeatures =
                        consolidatedVineyardWineFeatures.filter(
                          (wine) =>
                            wine.vineyardId === vineyard.id ||
                            wine.vineyardName === vineyard.name,
                        );
                            ...
                            <Card>
                              <CardHeader>
                                <CardTitle
                                  ...
                                  onClick={() =>
                                    toggleSection(
                                      `vineyard_wine_features_${vineyard.id}`,
                                    )
                                  }
                                >
                                  <Star className="h-4 w-4 text-purple-600" />
                              ...
                              {!isSectionCollapsed(
                                ...
                                <CardContent className="space-y-3">
                                  {vineyardWineFeatures.map(
                                    (consolidatedEvent, index) => (
                                      <div
                                        key={`${consolidatedEvent.vineyardId}_${consolidatedEvent.grape}_${consolidatedEvent.vintage}`}
                                      >
                                        <ConsolidatedWineFeatureDisplay
                          ...
                          <Card>
                            <CardHeader>
                              <CardTitle
                                className="flex items-center gap-2 text-base cursor-pointer hover:bg-gray-50 p-2 -m-2 rounded transition-colors"
                                onClick={() =>
                                  toggleSection(`vineyard_${vineyard.id}`)
                                }
                              >
                                <Grape className="h-4 w-4 text-green-600" />
                                {vineyard.name}
                            ...
                            {!isSectionCollapsed(`vineyard_${vineyard.id}`) && (
                              <CardContent className="space-y-3">
                                {vineyard.events.map((event, index) => (
                                  <div key={event.id}>
                                    <EventDisplay event={event} />
                                    {index < vineyard.events.length - 1 && (
                                      <Separator className="mt-3" />
                                    )}
                                  </div>
                                ))}
              ...
              <CardContent className="space-y-3 text-sm">
                ...
                <div className="space-y-2">
                  ...
                  <div className="ml-4 space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <strong>Company Value:</strong> Based on company value
                        (total assets - total liabilities) vs. max land value.
                        Uses logarithmic scaling for natural diminishing
                        returns.
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <strong>Sales:</strong> Temporary prestige from wine
                        sales. Decays at 5% weekly (95% retention rate).
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0"></div>
                      <div>
                        <strong>Wine Features (Company):</strong> Prestige from
                        wine features at company level. Can be positive
                        (terroir, bottle aging) or negative (oxidation, stuck
                        fermentation).
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <strong>Cellar Collection:</strong> Prestige from aged
                        wines (5+ years) in your cellar. Permanent source.
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-yellow-500 rounded-full mt-2 flex-shrink-0"></div>
                      <div>
                        <strong>Achievements:</strong> Special milestone
                        rewards. Decay rates vary by achievement type.
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="ml-4 space-y-2">
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <strong>Land Value:</strong> Permanent prestige based on
                        vineyard land value per hectare, adjusted for grape
                    ...
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <strong>Vine Age:</strong> Permanent prestige from vine
                        maturity, enhanced by grape suitability and density
                        modifiers.
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-purple-500 rounded-full mt-2 flex-shrink-0"></div>
                      <div>
                        <strong>Wine Features (Vineyard):</strong> Prestige from
                        wine features at vineyard level. Same features as
                        company level but calculated separately.
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      <div className="w-2 h-2 bg-emerald-500 rounded-full mt-2 flex-shrink-0"></div>
                      <div>
                        <strong>Vineyard Sales:</strong> Temporary prestige from
                        sales of wines from this specific vineyard. Decays at 5%
                        weekly.
                      </div>
                    </div>
                    <div className="flex items-start gap-2">
                      ...
                      <div>
                        <strong>Vineyard Achievements:</strong> Milestone
                        rewards specific to vineyard activities (planting,
                        aging, improvement, harvest).
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="ml-4 space-y-1 text-xs text-muted-foreground">
                    <div>
                      • <strong>Density Modifier:</strong> Lower vine density
                      (1500 vines/ha) = higher prestige (1.5×), higher density
                      (10000 vines/ha) = lower prestige (0.5×)
                    </div>
                    <div>
                      • <strong>Grape Suitability:</strong> Regional grape
                      suitability multiplies both land and age prestige
                    </div>
                    <div>
                      • <strong>Decay Rates:</strong> Sales decay at 5% weekly
                      (95% retention), some features decay at 2% weekly (98%
                      retention)
                    </div>
                    <div>
                      • <strong>Permanent Sources:</strong> Company value,
                      vineyard land/age, cellar collection, and achievements
                      don't decay
                    </div>
                    <div>
                      • <strong>Wine Features:</strong> Can be positive
                      (terroir, bottle aging) or negative (oxidation, stuck
                      fermentation) prestige
                    </div>
                  </div>
                </div>
              </CardContent>
