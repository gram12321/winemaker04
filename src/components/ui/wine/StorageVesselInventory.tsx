  ...
  return <div className="bg-white rounded-lg shadow overflow-hidden">
    ...
    <div className="p-3">
      {vessels.length === 0 ? <p className="text-xs text-gray-500">Buy casks from the market to build winery storage. Each purchase creates an individual vessel with fixed capacity.</p> : <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
        {vessels.map((vessel) => <div key={vessel.id} className="rounded border border-gray-200 p-2 text-xs"><div className="font-medium text-gray-800">{vessel.capacityLitres.toLocaleString()} L {vessel.material === 'oak' ? 'Oak' : vessel.material} {vessel.vesselType.replace('_', ' ')}</div><div className="mt-1 text-gray-500">Status: {vessel.occupancy.replace('_', ' ')}</div><div className="text-gray-500">Acquired for {formatNumber(vessel.acquisitionPrice, { currency: true, decimals: 0 })}</div></div>)}
      </div>}
    </div>
  </div>;
};
