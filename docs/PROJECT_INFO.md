...
| `staff` | Installed feature facade; `staffFeature` owns staff records, recruitment/factory logic, teams, competency, wages/founders, presentation, and Staff UI. Pure creation, competency, team-selection, and wage calculations load eagerly; persistence, finance workflows, and UI load only through feature operations. Activities consume Staff through this one-way seam. |
