...
| Achievement unlock, company | `achievement` | `null` | Tiered one-off: `+0.1`, `+3`, `+20`, `+100`, or `+300`. | Tier half-life formula; see achievement table. | `spawnAchievementPrestigeEvents()` in `src/lib/features/achievements/achievementService.ts`. |
...
Tiered achievements are generated by `createTieredAchievements()` in `src/lib/features/achievements/achievementDefinitionUtils.ts`.
