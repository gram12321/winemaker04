...
Current mainline repo state: active share-price services, active share-management UI, dividends, and weekly share-price snapshots described below are not wired in this repo. The repo currently contains share/board data scaffolding and an intentionally inactive `boardShare` facade. Local branch names indicate reintroduction work may exist outside this branch (`archive/board-share-active-seam`, `feature/readd-board-share`).
