...
- Sell-side grape buyers remain independent. The Buy Market owns a shared normalized offer/source/counterparty contract and registered-domain dispatcher for lifecycle and purchase coordination; one modal shell hosts the Grape Procurement and Storage Vessels panels. Local catalogues and global assets remain separate persistence mechanisms, while every adapter exposes the same seller/source/relationship presentation and keeps its own base pricing, projected state, and fulfilment rules. Storage Vessels combines company-scoped quantity supplier stock with canonical global used-vessel listings. Used listings retain the underlying asset's identity, material, age, condition, fills, and cleanliness; their condition and base value are deterministically projected for the viewer's game date before the buyer’s relationship multiplier is applied. The global adapter seam is ready for future grape lots and other goods; storage-vessel effects on produced wine remain deferred.
